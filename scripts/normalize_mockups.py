...
#
# Клиент 2026-08-29 вечером прислал СВОЙ эталон — `Champion синий фон.png` с боевого,
# тот самый, про который сказал «встал идеально, надо только чуть-чуть пониже опустить».
# Замер эталона: холст 900x1200, форма 754x992, сверху пусто 106 px (0.088), снизу 102 px
# (0.085) — то есть пустота у него поделена ПОРОВНУ, а не свалена вниз.
# У нас при 0.10 было: сверху 120, снизу 227..407. Отсюда 0.18 — при средней высоте формы
# 760 px верх и низ выравниваются (216 против 130..311), форма опускается, и заодно
# верхняя полоса растёт со 120 до 216 px под кнопки, которые он собирается добавлять.
# ⚠️ Пустоту это НЕ убирает, а перераспределяет: пара «перёд + зад» шире, чем выше
# (1.05..1.34 против холста 0.75), упирается ширина. Убрать пустоту можно только другой
# компоновкой исходника — у эталона она другая (крупный перёд + зад + гетры и бутсы).
TOP_MARGIN = 0.18
