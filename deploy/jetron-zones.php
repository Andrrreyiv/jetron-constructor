...
function jetron_zones_sanitize($data) {
    ...
    foreach ($data as $form_id => $boxes) {
        ...
        foreach ($boxes as $key => $box) {
            ...
            // Ручной цвет бренд-знака (клиент 04.09). Поле необязательное и есть только у записей
            // знака. Белый список выше пересобирает запись с нуля, поэтому без этой ветки цвет
            // молча пропадал бы при сохранении: на демо работает, на боевом исчезает.
            // Пропускаем строго #rrggbb — в JSON, который читает конструктор, мусору не место.
            if (isset($box['color']) && is_string($box['color'])
                && preg_match('/^#[0-9a-fA-F]{6}$/', $box['color'])) {
                $clean[$form_id][$key]['color'] = strtolower($box['color']);
            }
        }
    }
    return $clean;
