...
// Клиент 31.08 голосовым закрыл этот вопрос: «цифра должна касаться верхнего края рамки
// и нижнего края рамки, то есть там отступ один пиксель или 0 пиксель… сделать 0, а если
// не будет обрезаться, то так оставляем. Если будет всё-таки обрезаться на 0, тогда
// сделайте 1 пиксель». Проверено глазами: не обрезается, значит ровно ноль.
test('NUMBER_TOP_INSET_RATIO: отступа сверху нет — цифра касается верхней кромки', () => {
  assert.equal(NUMBER_TOP_INSET_RATIO, 0);
});
