...
test('подпись всегда «в карточку», даже когда адрес карточки не приехал', () => {
  // Клиент 31.08 (голосовое 11:06): «изначально я вам писал кнопку в карточку, надо обратно
  // вернуть в карточку. Или подробнее — но подробнее не надо, пускай люди знают, что это
  // переход в карточку». Это отменяет прежнюю осторожность с подписью «в каталог».
  //
  // Почему это не ложь на боевом: адрес карточки приходит из ответа WooCommerce на каждую
  // расцветку, и запасной путь в раздел линейки срабатывает только там, где карточки нет
  // вовсе (демо без каталога, чёрный Star). Оговорка про демо вписана в письмо №13.
  // `isCard` остаётся честным: подпись — это то, что видит покупатель, а флаг — то, что
  // знает код, и путать их нельзя.
