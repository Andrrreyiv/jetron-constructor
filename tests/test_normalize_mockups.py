...
class TestВерхняяПолоса:
    ...
    def test_полоса_вмещает_плашку_и_кнопку(self):
        assert CANVAS[1] * TOP_MARGIN > self.ВЫСОТА_ПЛАШКИ

    def test_высокая_форма_помещается_целиком(self):
        """Верхняя полоса плюс потолок высоты не длиннее холста. Кадр подобран так,
        что ограничивает высота, а не ширина, — на реальных 45 файлах всегда наоборот."""
        f = frame((0, 0, 580, 1000))
        assert f["top"] + f["height"] <= CANVAS[1]
