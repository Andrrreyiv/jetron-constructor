...
// Запас рамки отсечения. Замер 12.09 (клиент: «почему опять цифры обрезаются?»): у ДВУЗНАЧНЫХ
// номеров на спине чернила 140 px при рамке 140 — запас до боковых кромок ровно 0, у однозначных
// по высоте 1 px. Клип совпадал с зоной кромка в кромку и съедал сглаженные крайние пиксели.
// Сжать номер нельзя — край-в-край клиент просил сам (2f325f1, «номер край-в-край, стретч ≤15%»),
// поэтому запас отдан РАМКЕ ОТСЕЧЕНИЯ: размер цифры не меняется, срез уходит.
// Пиксели, а не доля: на мелком холсте доля вырождается в ноль — тот же разбор, что у отступа выше.
export const CLIP_SLACK_PX = 1;

// Прямоугольник отсечения по прямоугольнику зоны — с запасом со всех сторон.
export function clipRect(rect) {
  return {
    left: rect.left - CLIP_SLACK_PX,
    top: rect.top - CLIP_SLACK_PX,
    width: rect.width + 2 * CLIP_SLACK_PX,
    height: rect.height + 2 * CLIP_SLACK_PX
  };
}
...
//
// Суффикс `_dup` — дубль номера со спины на шортах. Клиент 12.09 (голосовое 15:29): «очень
// правильно сделано номер на спине — рамка, ты её двигаешь, и номер внутри пропорционально
// изменяется, так же бы на шортах». Раньше дубль под это правило не подпадал и жил бренд-механикой:
// админ тянул сам текст, сохранялся габарит текста, и подогнанная «7» не подходила под «22».
// `shorts_logo_dup` сюда НЕ попадает — он картинка, ему бренд-механика подходит.
export function isNumberZone(key) { return /(^|_)number(_dup)?$/.test(String(key || '')); }
